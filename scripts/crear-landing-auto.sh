...
if [ -z "$1" ]; then
    ...
    echo "Ejemplo: $0 \"Instalación de Minisplit\""
    echo ""
    exit 1
fi
...
cat > "servicios/${SLUG}/config.json" <<EOF
{
  ...
  "seo": {
    ...
    "keywords": "${SLUG} culiacan, electricista culiacan, servicio electrico"
  },
  "content": {
    "h1": "${SERVICE_NAME} en Culiacán | Profesional Certificado",
    "subtitle": "Servicio profesional de ${SERVICE_NAME} en Culiacán. Instalación rápida, garantía por escrito.",
...
TEMPLATE_SOURCE="servicios/reparacion-cortos-circuitos"
