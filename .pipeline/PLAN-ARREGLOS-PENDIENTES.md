...
- **Pendiente opcional futuro:** ~~testimonios idénticos~~ ✅ HECHO 2026-06-14 (commit `5aace84`): quitada la sección de testimonios anónimos idénticos de las 642 colonias. ~~consolidar index de colonias vs `/directorio/`~~ ✅ HECHO 2026-06-14 (commit `8db9ae8`): 301 de `/directorio/`→hub. ~~unificar CSS del blog~~ ✅ HECHO 2026-06-14 (commit `55a1f29`): blog/contacto/gracias pasan de `styles.min.css` (49KB sin hash) a `styles.7f293647.css` (36KB con hash); verificado pixel-idéntico con captura puppeteer; sw.js v8. `styles.min.css` queda sin referencias (retirable a futuro).
...
- 2026-06-14 · Testimonios · quitada la sección de testimonios anónimos idénticos en las 642 colonias (resto de la página intacto) · commit `5aace84` · publicado SÍ (push a main → Netlify) · verificación: 0 residuales, JSON-LD válido, validate-landing PASÓ, pre-commit OK.
- 2026-06-14 · CSS blog · 12 archivos (blog/contacto/gracias) de `styles.min.css`→`styles.7f293647.css` (hash, 36KB); sw.js v7→v8 · commit `55a1f29` · publicado SÍ (push a main → Netlify) · verificación: mismas 144 clases, reglas distintas no usadas por el blog, captura puppeteer antes/después PIXEL-IDÉNTICA (mismo SHA), paridad CSS OK.
