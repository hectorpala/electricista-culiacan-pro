...
if [ "${CLAUDE_OK:-0}" = 1 ]; then
  ...
  REASON=$(grep -m1 -iE "session limit|hit your|cuota|rate" "$LOG_DIR/auto-agente-$STAMP.log" 2>/dev/null || echo "error de la corrida")
  ...
  /usr/local/bin/node /Users/openclaw/gsc-mcp/send-report.mjs \
    "$FAILNOTE" "Auto Agente Electricista" "no completada" >> "$LOG_DIR/auto-agente-$STAMP.log" 2>&1 \
    || echo "[$STAMP] No se pudo enviar el aviso de falla (Auto Agente Electricista)." >> "$LOG_DIR/auto-agente-$STAMP.log"
fi
