...
# -*- coding: utf-8 -*-
"""
Checker DETERMINISTA de NAP (Name-Address-Phone) para electricistaculiacanpro.mx.
...
NAP canónico:
  NAME  = "Electricista Culiacán Pro"          (con acento)
  PHONE = 667 392 2273 / 526673922273
  EMAIL = contacto@electricistaculiacanpro.mx       (REGLAS f8c72299: el malo es un email de plomería (fuga))

Chequeos (sobre cada .html servido):
  1. EMAIL de plomería filtrado (fuga) `@plomero*`            -> alta   (REGLAS f8c72299)
  2. EMAIL del dominio bueno con local-part != "contacto"
     (p.ej. contacto@electricistaculiacanpro.mx)                -> media  (variante NAP)
  3. NOMBRE del negocio en JSON-LD (Organization/LocalBusiness…) != canónico -> media
  4. NOMBRE sin acento "Electricista Culiacan Pro" en el texto  -> media  (variante NAP)
...
NAME_OK = "Electricista Culiacán Pro"
NAME_NOACCENT = "Electricista Culiacan Pro"
...
def main():
    ...
    for fpath in paginas:
        ...
        # 1. email de PLOMERÍA filtrado (fuga) @plomero*
        bad = sorted(set(re.findall(r'[\w.+-]+@plomero[\w.-]*\.(?:com|mx)', t, re.I)))
        if bad:
            add("alta", r,
                "NAP: email con dominio INCORRECTO en %s: %s (el correcto es contacto@electricistaculiacanpro.mx — REGLAS f8c72299)" % (r, ", ".join(bad)),
                "Reemplazar por contacto@electricistaculiacanpro.mx en todas las apariciones")

        # 2. variante de local-part en el dominio bueno
        variantes = sorted(set(
            lp for lp in re.findall(r'([\w.+-]+)@electricistaculiacanpro\.mx', t, re.I)
            if lp.lower() != "contacto"))
        if variantes:
            add("media", r,
                "NAP: email con local-part VARIANTE en %s: %s@electricistaculiacanpro.mx (el canónico es contacto@)" % (r, ", ".join(v + "" for v in variantes)),
                "Unificar a contacto@electricistaculiacanpro.mx (consistencia NAP para SEO local)")

        # 3. nombre del negocio + teléfono en JSON-LD (dedup por página).
        #    Solo nombres "de marca" (contienen 'electricista') para no marcar a los
        ...
        for node in biz_nodes(t):
            nm = node.get("name")
            if (isinstance(nm, str) and "electricista" in nm.lower()  # electricista/electricidad…
        ...
        for nm in sorted(name_vars):
            if nm.startswith(NAME_OK):
                # "Electricista Culiacán Pro – Las Quintas": marca + sufijo de colonia.
        ...
        # 4. nombre sin acento en el texto (case-insensitive: "ELECTRICISTA CULIACAN PRO"
    ...
    # Patrón localizado "Marca – <Colonia>" (p.ej. "Electricista Culiacán Pro – Las Quintas"):
